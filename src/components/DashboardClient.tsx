...
// Total providers tracked in the index (including partial). Used for "N of 16" label.
const TOTAL_TRACKED = 16;

// Shared caveat shown wherever "Normalizing" previously appeared as a standalone state.
// Replaces scattered bespoke strings throughout the UI.
const DATA_CAVEAT =
...
function MarketRibbon({ listings, summary, activeProviders, totalListings }: {
  listings: GpuListing[]; summary: MarketSummary | null;
  activeProviders: number; totalListings: number;
}) {
  const items = useMemo(() => {
    ...
    const a100High = listings.filter(l => l.gpu_model.includes("A100") && l.availability === "high");
    const a100Spot = listings.filter(l => l.gpu_model.includes("A100") && l.pricing_type === "spot");
    const reliableAny = listings.filter(l => l.availability === "high").sort((a, b) => a.price_per_hour - b.price_per_hour)[0];
    const cheapestAny = [...listings].sort((a, b) => a.price_per_hour - b.price_per_hour)[0];

    // Prefer reliable (high-avail) price for ribbon; cheapest-observed is secondary label
    const h100Reliable = h100High.length ? Math.min(...h100High.map(l => l.price_per_hour)) : null;
    const h100Cheapest = h100Spot.length ? Math.min(...h100Spot.map(l => l.price_per_hour)) : null;
    const a100Reliable = a100High.length ? Math.min(...a100High.map(l => l.price_per_hour)) : null;
    const a100Cheapest = a100Spot.length ? Math.min(...a100Spot.map(l => l.price_per_hour)) : null;

    const providerLabel = activeProviders < TOTAL_TRACKED
      ? `${activeProviders} of ${TOTAL_TRACKED} providers tracked`
      : `${TOTAL_TRACKED} providers`;

    const base = [
      "AI Compute Market  ·  Live",
      ...(reliableAny ? [`Reliable from ${fmtP(reliableAny.price_per_hour)}/hr · ${getMeta(reliableAny.provider).short}`] :
      ...
      ...(h100Reliable !== null
            ...
            : h100Cheapest !== null
              ? [`H100 observed from ${fmtP(h100Cheapest)}/hr`]
              : ["H100: not in current snapshot"]),
      ...(a100Reliable !== null
            ? [`A100 reliable from ${fmtP(a100Reliable)}/hr`]
            : a100Cheapest !== null
              ? [`A100 observed from ${fmtP(a100Cheapest)}/hr`]
              : []),
      `${providerLabel} · ${totalListings.toLocaleString()} listings`,
      ...(summary?.last_updated ? [`Updated ${minsAgo(summary.last_updated)}`] : []),
    ];
    return [...base, ...base];
  }, [listings, summary, activeProviders, totalListings]);
...
function MarketBrief({ listings, summary, activeProviders }: { listings: GpuListing[]; summary: MarketSummary | null; activeProviders: number }) {
  const cheapestAny  = [...listings].sort((a, b) => a.price_per_hour - b.price_per_hour)[0];
  const cheapestHigh = listings.filter(l => l.availability === "high").sort((a, b) => a.price_per_hour - b.price_per_hour)[0];
  ...
  const briefItems = [
    ...
    cheapestH100High ? {
      ...
      // Fallback: cheapest reliable A100 if no H100 high-avail
      const a100High = listings.filter(l => l.gpu_model.includes("A100") && l.availability === "high")
        .sort((a, b) => a.price_per_hour - b.price_per_hour)[0];
      if (a100High) return {
        label: "H100 absent · A100 reliable from",
        value: `${fmtP(a100High.price_per_hour)}/hr`,
        sub: `${getMeta(a100High.provider).short} · high avail. — request audit for H100`,
        badge: "partial" as ConfidenceLevel,
        color: "var(--amber)",
      };
      return {
        label: "H100 coverage",
        value: h100All.length > 0 ? `${h100All.length} listings` : "Not in snapshot",
        sub: h100All.length > 0 ? "No reliability-verified listings" : "Request audit for current pricing",
        ...
        <div className="brief-grid" style={{ display: "grid", gridTemplateColumns: "1fr 316px", gap: 60, alignItems: "start" }}>
          ...
          <div>
            ...
            <p style={{ ...BODY, fontSize: 15.5, color: "var(--text-secondary)", lineHeight: 1.72, maxWidth: 540, fontWeight: 300 }}>
              Spot prices, hyperscaler premiums, and capacity signals across{" "}
              {activeProviders < TOTAL_TRACKED
                ? `${activeProviders} of ${TOTAL_TRACKED} tracked providers`
                : `${TOTAL_TRACKED} providers`}{" "}
              — so you can see what a quote hides before you commit.
            </p>
  ...
  const facts = [
    ...
    },
    {
      ...
      text: h100All.length > 0
        ...
        : "H100 data not yet available in this query window. " + DATA_CAVEAT,
      color: "var(--amber)",
    },
